...
- **Item System Overhaul**:
    - **Spawn Rate**: 아이템 생성 주기를 0.5초로 대폭 단축하여 더욱 빠른 템포의 게임플레이를 제공합니다.
    - **Key Probability**: 열쇠(Key) 획득 확률을 80%로 상향 조정했습니다.
    - **New Items**: 다양한 아이템 효과가 추가되었습니다.
        - **Double Shot**: 탄환이 2발씩 발사됩니다.
        - **Magnet**: 주변의 아이템을 자동으로 끌어당깁니다.
        - **Bomb (Red Key)**: 화면 내의 모든 적(Meteor)을 파괴하고 보스에게 큰 데미지를 입힙니다.
- **Bug Fixes**:
    - **Bomb Crash**: 폭탄 아이템 획득 시 게임이 멈추는 치명적인 오류를 수정했습니다.
    - **Spawn Timer**: 아이템이 생성되지 않던 타이머 로직 오류를 수정했습니다.
...
- [ ] **Mobile Optimization**: 모바일 터치 조작감 개선 (Backlog)
